...
const CheckoutPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setStripeElementsClientSecret(null);
    ...
    fetch('/api/create-payment-intent', {
      ...
      .catch(error => {
        if (!cancelled) {
          console.error('❌ [Checkout] Failed to initialize Stripe PaymentIntent:', error);
          setCheckoutError(error instanceof Error ? error.message : 'Card payment is temporarily unavailable.');
        }
      });
