  ...
  try {
    ...
    const itemsXml = products
      ...
      .map((product) => {
        ...
        const brand = escapeXml(product.brand || 'Cokaro');
...
<rss version="2.0" xmlns:g="http://base.google.com/ns/1.0">
  <channel>
    <title>Cokaro Google Merchant Center Feed${targetLabel}${currencyLabel}</title>
    <link>${BASE_URL}</link>
    <description>Selected Cokaro products for ${country || 'United Kingdom and United States'}${currencyLabel}</description>
    ${itemsXml}
  </channel>
</rss>`;
